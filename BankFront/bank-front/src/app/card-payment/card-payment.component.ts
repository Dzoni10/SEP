...
export class CardPaymentComponent implements OnInit {
  ...
  constructor(
    ...
    this.paymentForm = this.fb.group({
      pan: ['', [Validators.required, Validators.pattern(/^[\d\s]{13,23}$/)]],
  ...
  onSubmit(): void {
    ...
    this.http.post<any>('http://localhost:8082/api/v1/bank/process-payment', cardData)
      .subscribe({
        next: (response) => {
          this.loading = false;
          if (response.redirectUrl) {
            ...
            const successUrl = this.route.snapshot.queryParamMap.get('successUrl') || 'http://localhost:4200/payment-success';
            const failedUrl = this.route.snapshot.queryParamMap.get('failedUrl') || 'http://localhost:4200/payment-failed';
            if (response.success) {
        ...
        error: (error) => {
          ...
          if (error.error?.redirectUrl) {
            ...
            const errorUrl = this.route.snapshot.queryParamMap.get('errorUrl') || 'http://localhost:4200/payment-error';
            setTimeout(() => {
